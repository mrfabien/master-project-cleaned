...
class Storm_eu_square_modelleur(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(QgsProcessingParameterFileDestination('Square_eu_csv', 'square_eu_csv', fileFilter='Microsoft Excel (*.xlsx);;Open Document Spreadsheet (*.ods)', createByDefault=True, defaultValue=''))
        self.addParameter(QgsProcessingParameterFileDestination('Square_non_eu_csv_qgis_friendly', 'square_non_eu_csv_QGIS_friendly', fileFilter='Microsoft Excel (*.xlsx);;Open Document Spreadsheet (*.ods)', createByDefault=True, defaultValue=None))
        self.addParameter(QgsProcessingParameterFileDestination('Square_non_eu_csv', 'square_non_eu_csv', fileFilter='Microsoft Excel (*.xlsx);;Open Document Spreadsheet (*.ods)', createByDefault=True, defaultValue=None))
        ...
        self.addParameter(QgsProcessingParameterFeatureSink('Square_non_eu_', 'square_non_eu_', type=QgsProcessing.TypeVectorAnyGeometry, createByDefault=True, defaultValue=None))
    ...
    def processAlgorithm(self, parameters, context, model_feedback):
        ...
        # Extraire par localisation dans EU
        alg_params = {
        ...
        outputs['ExtraireParLocalisationDansEu'] = processing.run('native:extractbylocation', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        results['Square_eu_'] = outputs['ExtraireParLocalisationDansEu']['OUTPUT']
        ...
        alg_params = {
            ...
            'LAYERS': outputs['ExtraireParLocalisationDansEu']['OUTPUT'],
            'OVERWRITE': True,
            'USE_ALIAS': False,
        ...
        # Extraire par localisation en dehors EU
        alg_params = {
            'INPUT': outputs['TamponCarrDe4']['OUTPUT'],
            'INTERSECT': parameters['limite_eu'],
            'PREDICATE': [2],  # est disjoint
            'OUTPUT': parameters['Square_non_eu_']
        }
        outputs['ExtraireParLocalisationEnDehorsEu'] = processing.run('native:extractbylocation', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        results['Square_non_eu_'] = outputs['ExtraireParLocalisationEnDehorsEu']['OUTPUT']
        ...
        alg_params = {
            ...
            'INPUT': outputs['ExtraireParLocalisationDansEu']['OUTPUT'],
            'OUTPUT': QgsProcessing.TEMPORARY_OUTPUT
        }
        ...
        alg_params = {
            ...
            'FIELD_TYPE': 0,  # Décimal (double)
            'FORMULA': 'round(lon_west + 360,6)',
            'INPUT': outputs['ExtraireParLocalisationEnDehorsEu']['OUTPUT'],
            'OUTPUT': QgsProcessing.TEMPORARY_OUTPUT
        }
        outputs['CalculatriceDeChampLon_west'] = processing.run('native:fieldcalculator', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        ...
        alg_params = {
            ...
            'OVERWRITE': True,
            'USE_ALIAS': False,
            'OUTPUT': parameters['Square_non_eu_csv']
        }
        outputs['ExporterVersUnTableur'] = processing.run('native:exporttospreadsheet', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        results['Square_non_eu_csv'] = outputs['ExporterVersUnTableur']['OUTPUT']
        ...
        alg_params = {
            'FORMATTED_VALUES': False,
            'LAYERS': outputs['ExtraireParLocalisationEnDehorsEu']['OUTPUT'],
            'OVERWRITE': True,
            'USE_ALIAS': False,
            'OUTPUT': parameters['Square_non_eu_csv_qgis_friendly']
        }
        outputs['ExporterVersUnTableur'] = processing.run('native:exporttospreadsheet', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        results['Square_non_eu_csv_qgis_friendly'] = outputs['ExporterVersUnTableur']['OUTPUT']
        return results
